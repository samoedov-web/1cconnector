...
Методологическая оговорка (ст. 282.3 НК): переоценка — БУХГАЛТЕРСКАЯ
(достоверность отчётности/управленка), для налога на прибыль цифровая
валюта не переоценивается и «курсовых разниц» не образует — в налоговый
регистр (reports/tax.py) переоценка не включается ни при каких настройках.
Термин «курсовые разницы» к цифровой валюте не применяется — это разницы
оценки имущества. Порядок бухгалтерского отражения утверждает методолог/
аудитор клиента (документ 1С приходит проектом).
