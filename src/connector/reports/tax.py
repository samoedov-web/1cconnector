...
ВАЖНО (ст. 282.3 НК): в регистр входят ТОЛЬКО реализованные операции.
Переоценка остатков (accounting/revaluation.py) — бухгалтерская/управленческая
и в налоговую базу не включается: цифровая валюта для налога на прибыль
не переоценивается, «курсовых разниц» не образует (это не иностранная
валюта). Инвариант закреплён тестом.
